...
**問題生成ロジックはここには無い。** バックエンド（Java / Spring Boot）へ移行済み。
暫定実装だった `src/utils/problemGenerator.ts` は削除した。

問題タイプの一覧も問題そのものも、すべて `src/api.ts` 経由でサーバーから取得する。
**サーバーが起動していないと、内容選択より先へ進めない。**
...
src/
  ...
  pages/        1つのURLに対応する画面
  api.ts        サーバーとのやりとり。URLとエラー判定をここに集約する
...
**画面から直接 `fetch` を書かない。** 必ず `api.ts` に関数を足して、そこを呼ぶ。
サーバーのアドレスが変わったときに全画面を直すことになるため。

---

## ページ間のデータ受け渡し

**`location.state` は使わない。URLのクエリパラメータに載せる。**
...
理由：`state` はブラウザのメモリ上にしかないため、**リロードで消える**。
URLに載せておけば再現でき、設定を人に送れる。

そのかわり、**URLから来た値は必ず検証する**。誰でも書き換えられるため。
数値は `Number.isInteger()` で、文字列は候補との比較か、サーバーに問い合わせて
存在するかで確かめる。

## 通信中の画面

`fetch` する画面は必ず3つの状態を持つ。

```
loading   読み込んでいます…
error     取得できませんでした（+ 戻るリンク）
ready     本体
```

**エラー画面には必ず戻る手段を置く。** 行き止まりを作らない。
詳しい内容は `console.error` に出し、画面には短く出す。
